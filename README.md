...
最近 DeepSeek 推出了 **DeepSeek V4 Flash Vision Exp**。但视频是「连续的画面 + 声音」——"只能读图怎么行？如果读视频呢？"于是有了 VidSense：把视频拆成 **声音（转写）+ 画面（关键帧）**，交给视觉大模型综合理解，返回这段视频到底讲了什么。
...
- **输入**：本地视频文件（mp4/mov/...）或 **B站视频 URL**（`bilibili.com/...`、`b23.tv/...`）。
  > 注：项目核心是「理解视频」；B站链接下载只是**辅助入口**，接口仍在完善中。
- **本地理解（事实依据）**：
...
**下方实验证明模型读的是真实像素，而非脑补。**

---

## 🧪 防幻觉验证：对比实验（证明模型读的是真实像素）

做法：**保持同一段字幕完全不变，只替换发给模型的关键帧图片**，对比模型描述是否跟随像素。

**实验 A（只发文字，无图）** —— 模型只能靠台词脑补场景：
> "视频记录了一段车内对话……车主发现有人坐在自己车里……"
> （车内的画面细节无从验证，只能从台词推断）

**实验 B（同一字幕 + 换成"彩色测试卡"帧）** —— 模型改口描述真实像素：
> "这段视频视觉上全程显示的是彩色测试卡（SMPTE color bars）图案……画面与音频内容完全不匹配，表明这可能是一个测试视频、素材占位符。"

**为什么这证明"真看到了"：**
- "SMPTE color bars / 彩色测试卡 / 彩条图案"——**字幕里一个字都没有**，只能来自像素。
- 若模型在脑补，实验 B 应和实验 A 一样说"车主/车里"（因为字幕相同）；但它**改口描述了测试卡**，还**主动指出"画面与音频不匹配"**——只有真正读了像素才做得到。
- 结论：画面内容来自**真实关键帧的像素**；转写 + 关键帧的 grounding 把幻觉压到最低。模型仍可能在其他处脑补（如实验 A 靠台词推断），但**视觉内容是如实读出的**。
